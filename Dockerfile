...
RUN npx tsc --target ES2021 --module commonjs --skipLibCheck --esModuleInterop scripts/fix-failed-airdrop-phase2.ts --outDir dist/scripts
...
CMD ["sh", "-c", "\
    ...
    node dist/scripts/fix-failed-airdrop-phase2.js && \
    node dist/src/main \
"]