...
describe('GetMarketplaceListings', () => {
  let handler: GetMarketplaceListings;
  ...
  beforeEach(() => {
    ...
    handler = new GetMarketplaceListings(
  ...
  describe('execute', () => {
    it('should return empty listings when no blockchain listings', async () => {
      mockBlockchain.getAllActiveMarketplaceListings.mockResolvedValue([]);
      const result = await handler.execute();
      expect(result).toEqual({ total: 0, listings: [] });
    });

    it('should return paginated empty when no blockchain listings with pagination', async () => {
      mockBlockchain.getAllActiveMarketplaceListings.mockResolvedValue([]);
    ...
    it('should filter seller listings by THB payment token', async () => {
      mockBlockchain.getAllActiveMarketplaceListings.mockResolvedValue([
        {
          listingId: '1',
      ...
      // After filtering to seller-only, only listing 1 remains
      mockPrisma.$queryRaw
    ...
    it('should skip listings without DB detail', async () => {
      ...
      mockPrisma.$queryRaw
        ...
        .mockResolvedValueOnce([]); // no counts
        ...
        .mockResolvedValueOnce([
          {
            ...
            voucherValue: 50,
            voucherStartDate: null,
            voucherEndDate: new Date(Date.now() - 86400000), // past
            voucherStatus: 'active',
            voucherMerchantId: 'm1',
            voucherSellerMerchantId: null,
            merchantId: 'm1',
    ...
    it('should filter by merchantId when provided', async () => {
        ...
        .mockResolvedValueOnce([
          {
            ...
            voucherEndDate: new Date(Date.now() + 86400000),
            voucherStatus: 'active',
            voucherMerchantId: 'other-merchant',
            voucherSellerMerchantId: null,
      ...
      const result = await handler.execute('my-merchant'); // different merchant
    ...
    it('should handle pagination correctly', async () => {
      ...
      const result = await handler.execute(undefined, false, 1, 2);
    ...
    it('should filter out expired-status vouchers', async () => {
      mockBlockchain.getAllActiveMarketplaceListings.mockResolvedValue([
        {
          listingId: '1',
          paymentToken: '0xtoken',
        ...
        .mockResolvedValueOnce([
          {
            ...
            voucherStartDate: null,
            voucherEndDate: new Date(Date.now() + 86400000),
            voucherStatus: 'expired', // expired status
            voucherMerchantId: 'm1',
            voucherSellerMerchantId: null,
            merchantId: 'm1',
            merchantName: 'M',
    ...
    it('should filter out sold-out listings', async () => {
      mockBlockchain.getAllActiveMarketplaceListings.mockResolvedValue([
        {
          listingId: '1',
          paymentToken: '0xtoken',
      ...
      mockPrisma.$queryRaw
        ...
        .mockResolvedValueOnce([]); // no available counts
